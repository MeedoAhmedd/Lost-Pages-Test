...
// ---------- lighting ----------
// Night-time mood: cool deep-indigo ambient base, so the warm practicals (lamps/screens)
// read as pockets of amber light rather than everything being evenly, flatly lit.
scene.add(new THREE.HemisphereLight(0xaab0e6, 0x241f30, 0.5));
scene.add(new THREE.AmbientLight(0x3a3550, 0.22));

// "warm" is the key light (moonlight-through-window feel, warmed at the bounce) —
// dropped from 1.45 to keep it a gentle key rather than flat daylight, and pushed a touch
// more amber so it still visually agrees with the practical lamps.
var warm = new THREE.DirectionalLight(0xffd9a8, 1.05);
...
// cool fill — deep indigo/violet, the "night outside" bounce
var cool = new THREE.DirectionalLight(0x8a7aff, 0.5);
cool.position.set(-6, 5, -6);
scene.add(cool);

// top fill — was flat neutral white (daylight-ish); recoloured cool/indigo and dimmed
// so it no longer competes with the warm practicals for the room's "identity" light.
var topFill = new THREE.DirectionalLight(0x6a7bb0, 0.16);
...
var M = {
  // roughness nudged up on floor/wood so they read as matte, warm wood catching lamp-light
  // rather than a slightly glossy/plastic surface — color values themselves already read
  // as warm wood and deep charcoal/indigo walls, so those are left alone.
  floor : mat(0x8a6045, 0.82),
  wall  : mat(0x413a4c, 0.95),
  wallB : mat(0x4a4256, 0.95),
  rim   : mat(0xf2eee9, 0.85),
  wood  : mat(0x5b3f2c, 0.8),
  woodL : mat(0x8a6647, 0.75),
...
// cool moonlight spill through the window — kept blue/cool (it IS the "darkness outside"
// light source) but dialed back a little so it doesn't outcompete the room's warm practicals.
var winLight = new THREE.PointLight(0x9fb8ff, 0.85, 12);
...
(function deskLamp(){
  ...
  // slightly stronger now that the room's general ambient is dimmer/cooler — this small
  // lamp needs to still read clearly as a warm pool of light on the desk.
  var glow = new THREE.PointLight(0xffdca8, 0.85, 2.6, 2);
...
// was a flat 0x0c0c10 (fully inert, "off" black) — unlike the TV/monitor this screen has no
// animated canvas, so it read as a dead gray-black rectangle. MeshBasicMaterial ignores scene
// lighting and always shows its own color, so lifting that color to a dim cool-white gives it
// a believable soft "screen glow" for free — no extra texture, canvas, or light needed.
var laptopScreenMat = new THREE.MeshBasicMaterial({ color:0x2a3138 });
(function laptopUnit(){
  var lg = new THREE.Group();
...
/* ---- soft overhead room light (invisible source, no fixture) ----
   Was a broad, fairly bright (1.0 @ radius 16) neutral-warm fill that lit the whole
   room evenly, working against the "pockets of warmth" goal. Pulled back to a smaller,
   dimmer glow so the desk lamp / side lamp / screens read as the room's actual light
   sources and this just keeps far corners from going fully black. */
var roomLight = new THREE.PointLight(0xffe9cf, 0.5, 10, 2);
